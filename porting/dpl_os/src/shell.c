...
void *
run_stdin_shell(void *p)
{
    const struct shell_cmd *cmd;
    struct termios oldt, newt;

    /* Set stdin/out to character-mode instead of line-mode */
    tcgetattr(STDIN_FILENO, &oldt);
    newt = oldt;
    newt.c_lflag &= ~(ICANON|ECHO);
    newt.c_cc[VMIN]=1;
...
static struct dpl_eventq *shell_evq;
/* Queue for available shell events */
static struct dpl_event shell_console_ev[MYNEWT_VAL(SHELL_MAX_CMD_QUEUED)];
static struct console_input buf[MYNEWT_VAL(SHELL_MAX_CMD_QUEUED)];

void
shell_evq_set(struct dpl_eventq *evq)
{
    shell_evq = evq;
...
shell_init(void)
{
    ...
    shell_evq_set(dpl_eventq_dflt_get());
    ...
    pthread_create(&var_t1, NULL, run_stdin_shell, NULL);
}
